...
public class Room {
	...
	public int randomDonkeyIndex() {// dá um indice aleatório de um Donkey Kong na lista dos Interactable, retorna
									// -1 se não houver gorilas no mapa
	...
	public boolean arroundManel(Interactable i) {
		...
		if (down.equals(i.getPosition()) || up.equals(i.getPosition()) || right.equals(i.getPosition())
				|| left.equals(i.getPosition())) {
			return true;
		}
		return false;
	}
	...
	public boolean canMove(Movable ob, Direction d) {
		...
		if (insideMap(nextPosition) && !isBlock(nextPosition) && !isActivatedBomb(nextPosition)
				&& !isDonkeyKong(nextPosition) && !manel.getPosition().equals(nextPosition)) {
			if (ob.getName().equals("DonkeyKong") && isDoor(nextPosition)) {
				return false;
			}

			if (d.name() == "UP" && !isStairs(ob.getPosition())) {// Caso o objeto nao esteja numa escada não pode subir
																	// de posição ou seja "voar"!
				return false;
			}
	...
	public void manelStatus() {
		...
		if (manel.hasBomb()) {
			ImageGUI.getInstance().setStatusMessage("Vidas: " + manel.getLives() + "  Saúde: " + manel.getHealth()
					+ "  Dano: " + manel.getDamage() + "  💣");
		} else {
			ImageGUI.getInstance().setStatusMessage(
					"Vidas: " + manel.getLives() + "  Saúde: " + manel.getHealth() + "  Dano: " + manel.getDamage());
		}
